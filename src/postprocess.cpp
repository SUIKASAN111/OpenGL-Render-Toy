...
void PostProcess::EndRender()
{
	...
	glBlitFramebuffer(0, 0, write_rt->width, write_rt->height, 0, 0, read_rt->width, read_rt->height, GL_COLOR_BUFFER_BIT, GL_NEAREST);
...
void BloomProcess::Execute(unsigned int quad)
{
    ...
    glBindTexture(GL_TEXTURE_2D, pingpong_buffer[!horizontal]->color_buffer);
