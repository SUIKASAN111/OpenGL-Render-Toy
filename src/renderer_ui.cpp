...
{
    int width = window->Width() / 7 > 200 ? window->Width() / 7 : 200;
    int height = window->Height() / 6 > 80 ? window->Width() / 6 : 80;
    ...
    window_flags = ImGuiWindowFlags_MenuBar | ImGuiWindowFlags_NoCollapse;
    {
        ...
        ImGui::SeparatorText("Rendering Setting");
        {
            ...
            ImGui::Checkbox("Enable Skybox", &EditorSettings::SkyboxEnabled);
            ImGui::SetNextItemWidth(150);
            ImGui::DragFloat("shadow distance", &scene->render_pipeline.shadow_map_setting.shadow_distance);
        }
...
void renderer_ui::sceneUI(RendererWindow *window, Scene *scene)
{
    ...
    int width = window->Width() / 7 > min_width ? window->Width() / 7 : min_width;
    int main_height = window->Height() / 6 > 80 ? window->Width() / 6 : 80;
